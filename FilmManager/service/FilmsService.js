...
exports.getPublicFilmsTotal = function() {
  return new Promise((resolve, reject) => {
      const sql = "SELECT count(*) total FROM films f WHERE private = 0 ";
      db.get(sql, [], (err, size) => {
          if (err) {
              reject(err);
          } else {
...
exports.getPublicFilms = function(req) {
  return new Promise((resolve, reject) => {
    let sql = "SELECT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite, c.total_rows "
              + "FROM films f, (SELECT count(*) total_rows FROM films l WHERE l.private=0) c "
              + "WHERE  f.private = 0 "
    ...
    db.all(sql, limits, (err, rows) => {
        if (err) {
            ...
            const films = rows.map((row) => createFilm(row));
            resolve(films);
        }
    });
  });
...
exports.getPrivateFilmsTotal = function(userId) {
  return new Promise((resolve, reject) => {
      const sql = "SELECT count(*) total FROM films f WHERE private = 1 AND owner = ? ";
      db.get(sql, [userId], (err, size) => {
          if (err) {
              reject(err);
          } else {
              resolve(size.total);
...
exports.getPrivateFilms = function(req) {
  return new Promise((resolve, reject) => {
      ...
      let sql = "SELECT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite, c.total_rows "
                + "FROM films f, (SELECT count(*) total_rows FROM films l WHERE l.private=1 AND owner = ?) c "
                + "WHERE  f.private = 1 AND owner = ?"
      const limits = getPagination(req);
      if (limits.length != 0) sql = sql + " LIMIT ?,?";
      let parameters = [req.user.id, req.user.id];
...
exports.getInvitedFilmsTotal = function(reviewerId) {
  return new Promise((resolve, reject) => {
      const sql = "SELECT count(*) total FROM films f, reviews r WHERE  f.private = 0 AND f.id = r.filmId AND r.reviewerId = ? ";
      db.get(sql, [reviewerId], (err, size) => {
          if (err) {
              reject(err);
          } else {
              resolve(size.total);
...
exports.getInvitedFilms = function(req) {
  return new Promise((resolve, reject) => {
    let sql = "SELECT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite, c.total_rows FROM films f, reviews r, (SELECT count(*) total_rows FROM films f2, reviews r2 WHERE f2.private=0 AND f2.id = r2.filmId AND r2.reviewerId = ?) c WHERE  f.private = 0 AND f.id = r.filmId AND r.reviewerId = ?"
    let limits = getPagination(req);
    if (limits.length != 0) sql = sql + " LIMIT ?,?";
    limits.unshift(req.user.id); //TODO: check if this is correct
  ...
  return new Promise((resolve, reject) => {
      ...
      db.all(sql1, [filmId], (err, rows) => {
          if (err)
            ...
            let sql2 = 'UPDATE films SET title = ?';
            let parameters = [film.title];
  ...
  return new Promise((resolve, reject) => {
      ...
      db.all(sql1, [filmId], (err, rows) => {
          if (err)
              ...
              const sql2 = 'DELETE FROM films WHERE id = ?';
              db.run(sql2, [filmId], (err) => {
                  if (err)
                       reject(err);
                   else
